#!/usr/bin/env python3
"""
Module 3-posterior, calculate the posterior probability for the
various hypothetical probabilities of developing severe side
effects given the data.
i.e. P(A|B) in Bayes' rule P(A|B) = P(B|A)*P(A)/P(B)
"""
import numpy as np
...
def posterior(x, n, P, Pr):
    ...
    for i in range(2, n - x + 1):
        n_choose_x //= i
    L = float(n_choose_x) * P ** x * (1 - P) ** (n - x)

    return L * Pr / np.sum(L * Pr)
